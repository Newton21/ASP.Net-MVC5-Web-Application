    ...
    public class DepartmentController : Controller
    {
        ...
        public ActionResult Create([Bind(Include="DepartmentId,DepartmentCode,DepartmentName")] Department department)
        {
            if (ModelState.IsValid)
            {
                ...
                if (save > 0)
                {
                    this.ShowMessage(MessageType.Success, "Department Saved Successfully", true);
                }
        ...
        public JsonResult CheckDepatmentCode(string departmentCode, string initialCode)
        {
            var result = db.Departments.Count(d => d.DepartmentCode == departmentCode) == 0;
            return Json(result, JsonRequestBehavior.AllowGet);
        }

        public JsonResult CheckDepatmentName(string departmentName)
        {
